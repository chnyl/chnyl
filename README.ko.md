...
#### 💹 자율 금융 AI Agent (2026.04~ 진행 중)

> Kronos AI + NFI Rules. 바이낸스 선물. 24시간 자율 매매.

**핵심 아키텍처:**
- **Kronos** (AAAI 2026 채택) — 금융 캔들스틱 전용 AI 파운데이션 모델. 바이낸스 공개 API로 1시간봉 기반 24시간 가격 예측 (무료).
- **NFI (NostalgiaForInfinity)** — GitHub 별 3,000+ 규칙 기반 전략. RSI, MFI, EMA 크로스, MACD 신호 종합.
- **모듈식 설계** — `use_kronos` 토글: AI+규칙 (보수적, 둘 다 동의 필요) 또는 규칙만 (독립 동작, AI 서버 불필요).
- **원클릭 배포** — `start-all.sh` / `stop-all.sh`로 경로 자동 인식. 매시간 cron 예측. Cloudflare 터널로 대시보드 원격 접속.

**관련 오픈소스 빌딩 블록 (학습 및 기여):**

* **[freqtrade-nfi-bot](https://github.com/chnyl/freqtrade-nfi-bot)** — Kronos+NFI 하이브리드 자동매매 시스템. 모듈식 전략, 바이낸스 선물, 텔레그램 알림, 원클릭 배포.
* **[open-trading-api](https://github.com/chnyl/open-trading-api)** — LLM 자동화 환경에 최적화된 한국투자증권 KIS Open API 샘플 코드. 전략 설계 → 백테스터 → MCP 기반 실주문 실행까지 전체 파이프라인 학습 중.
* **[vectorbt](https://github.com/chnyl/vectorbt)** — Pandas/NumPy/Numba 기반 벡터화 백테스팅 라이브러리. 멀티 에셋 전략 프로토타이핑 및 인터랙티브 HTML 리포트 생성 실험 중.
* **[Macro-Pulse](https://github.com/chnyl/Macro-Pulse)** — 거시 지표와 지수 히트맵을 종합해 HTML 리포트를 생성하고 텔레그램으로 자동 발송하는 매크로 모니터링 봇. GitHub Actions 스케줄 자동화 적용.
* **[dexter](https://github.com/chnyl/dexter)** — 복잡한 금융 질문을 구조화된 리서치 플랜으로 분해하고, 실시간 시장 데이터를 수집·자기검증하는 자율 금융 리서치 Agent (TypeScript/Bun 기반).
* **[hermes-agent](https://github.com/NousResearch/hermes-agent)** — 코어 플랫폼. Self-Evolution 스킬 시스템, 크로스 세션 메모리, 멀티 에이전트 워크플로우.
...
#### 🧠 Second Brain / Knowledge Ontology (2026.04~ 진행 중)

> 지식 연결을 설계하다. 사고를 진화시키다.

**핵심 아이디어:** 세션, 메모리, 스킬, Obsidian 노트를 그래프로 연결하는 진화하는 Personal DB

- **분산 저장**: 세션(대화), 메모리(설정), 스킬(재사용 가능한 스킬), obsidian-notes(지식)
- **자동 연결**: 모든 노드 간 참조 네트워크
- **지속적 진화**: 작업 해결 → 스킬 저장 → 자동 로딩 → 점진적 학습
...
#### 💻 [Samsung Electronics PSU Calculator](https://samsungpsu.vercel.app/) ('26.2 배포 완료)

> **PSU 가치 시뮬레이션 웹 애플리케이션** (React 19 + TypeScript 기반)

- **주요 기능**: 추정 주가 기반 0x ~ 2.0x PSU payout 배수 자동 계산, 레벨별(CL1-2, CL3-4) 모델링, KST 기반 캐싱.
- **Vercel Serverless Functions**: 무료 외부 프록시의 대역폭 제한 및 CORS 이슈를 극복하기 위해 커스텀 API 엔드포인트를 구축하여 서비스 안정성 확보.
- **Vercel Edge Caching**: Yahoo Finance API의 빈번한 호출로 인한 Rate Limit 문제를 극복하기 위해 12시간 Edge Caching을 적용, 응답 속도 최적화 및 외부 API 의존도 감소.
- **Bundle Optimization**: 무거운 서드파티 라이브러리 의존성(radix, axios, recharts 등)을 대거 제거하고 순수 React로 성능 최적화, Google AdSense 연동 완료.
