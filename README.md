...
- **🌐 분산 저장층**: 세션마다 생성되는 `sessions/` 폴더에 전체 대화 기록이 저장되고, `memories/`에 크로스세션 메모리(사용자 선호도, 설정 등)가 유지되며, `skills/`에 재사용 가능한 스킬 라이브러리가 축적되고, `obsidian-notes/`에 정리된 지식 노트가 Git으로 버전관리 된다.

- **🔗 동적 연결망**: 스킬과 세션, 메모리와 파일, Obsidian 노트 간에 자동 참조 네트워크가 형성되어 모든 지식이 서로 연결된다. 예를 들어, 특정 세션에서 생성한 인사이트가 자동으로 관련 스킬과 Obsidian 노트에 역링크된다.

- **🔄 Self-Evolution**: 해결한 작업을 스킬로 저장하면 다음 세션에서 자동 로드되고, 쓸수록 똑똑해지는 피드백 루프가 작동한다. 매 세션마다 Agent가 학습하여 나을어진다.

- **🎯 Multi-Modal**: 텍스트 대화, 코드 조각, JSON 데이터, 이미지 분석 결과, API 응답, 금융 정보 등 모든 형식의 데이터를 지식 노드로 통합 저장한다. 형식에 구애받지 않는다.

- **🚀 영구 보존**: Git 버전관리와 Obsidian Vault를 활용해 모든 지식 그래프를 안전하게 보관하고, 시간순으로 시각화할 수 있다. 영구적이고 추적 가능하다.
